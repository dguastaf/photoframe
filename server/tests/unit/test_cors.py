...
def test_cors_origins_parsed_from_comma_separated_env():
    parsed = Settings(cors_origins="http://a.test:3000, http://b.test:4000")
...
def test_cors_origins_default_to_client_dev_origin_from_ports(monkeypatch):
    ...
    parsed = Settings(client_dev_host="frame.local", client_dev_port=6389)
    assert parsed.cors_origins == ["http://frame.local:6389"]


def test_cors_preflight_allows_configured_origin():
    client = TestClient(app)
    origin = settings.client_dev_origin
